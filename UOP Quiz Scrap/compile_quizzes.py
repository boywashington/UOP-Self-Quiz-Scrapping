...
import re
import shutil
import stat  # Added for modifying file permissions on Windows 11
...
INDEX_HTML = "Quiz_Unit-2_CS-4402-01.html"
...
def remove_readonly(func, path, excinfo):
    """
    Windows 11 / OneDrive Exception Recovery: 
    Clears the read-only attribute bit on locked file structures and retries deletion.
    """
    try:
        os.chmod(path, stat.S_IWRITE)
        func(path)
    except Exception:
        pass  # If it still fails, allow the program to exit gracefully without crashing
...
async def main():
    ...
    # Always run download if the workspace directory doesn't exist or is completely empty
    ...
    if unique_data:
        # 1. Compile questions to final PDF
        build_pdf_document(unique_data, OUTPUT_PDF)
        
        # 2. Automated Cleanup Phase (Updated for Windows 11 / OneDrive)
        if os.path.exists(TEMP_HTML_DIR):
            print(f"\nCleaning up dynamic workspace folder '{TEMP_HTML_DIR}' and all its contents...")
            
            # Give background file indexing/OneDrive locks 1 full second to release handles
            await asyncio.sleep(1.0)
            
            try:
                # Use custom onerror callback to reset Windows file permissions dynamically
                shutil.rmtree(TEMP_HTML_DIR, onerror=remove_readonly)
                
                if not os.path.exists(TEMP_HTML_DIR):
                    print("Temporary folder and its content cleaned up successfully.\n")
                else:
                    print(f"  Warning: Folder '{TEMP_HTML_DIR}' is locked by another process. You can safely delete it manually later.")
            except Exception as e:
                print(f"  Warning: Could not delete temporary files directory automatically. Error: {e}")
    else:
